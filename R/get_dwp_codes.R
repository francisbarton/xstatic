...
get_dwp_codes <- function(
  ...
  bens_list <- dwp_schema() %>%
    pull(label)

  # try to match ben to a full dataset name
  ben <- str_subset(string = bens_list, pattern = ben)

  assert_that(is.string(ben), msg = "A single dataset name was not matched.")
  ...
  if(chatty) {
    usethis::ui_info(paste0("Choosing ", default, "option: ", ds))
  ...
  if(chatty) {
    usethis::ui_info(paste("Period id is:", period_id))
  }
  ...
  if(chatty) {
    usethis::ui_info(paste("Geography type id is:", geo_type_id))
  }
  geo_field_id <- sx_pull_id(geo_field, geo_type_id)
  if(chatty) {
    usethis::ui_info(paste("Geography field id is:", geo_field_id))
  }

  geo_level_id <- sx_pull_id(geo_level, geo_field_id)
  geo_level_label <- dwp_schema(geo_field_id) %>%
      slice(geo_level) %>%
      pull(label)

  if(chatty) {
    usethis::ui_info(paste("Geography level id is:", geo_level_id))
    usethis::ui_info(paste("Geography level is:", geo_level_label))
  }
...
# these are all efectively wrappers around Evan Odell's `dwp_schema`
...
sx_get_measure <- function(id, type = "VALUESET") {
  sx_get_db(nm = nm, db = db) %>%
    sx_pull_id(slice = ms)
}
sx_get_geog <- function(nm, db = 1, ms, type, level) {
  sx_get_measure(nm = nm, db = db, ms = ms) %>%
    sx_pull_id(slice = type) %>%
    sx_pull_id(slice = level)
}
